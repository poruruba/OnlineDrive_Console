...
var vue_options = {
    ...
    methods: {
        set_apikey: function(){
            ...
            localStorage.setItem("dropbox_api_key", apikey);
            alert("リロードしてください。");
        },
        ...
        start_token_refresh: async function(){
            try{
                var input = {
                    url: base_url + "/dropbox/token-refresh",
                    api_key: this.apikey
                };
                var result = await do_http(input);
                console.log(result);
        ...
        start_authorize: async function(){
            ...
            new_win = open(base_url + '/dropbox-login' + '?' + to_urlparam(params), null, 'width=600,height=750');
        },
    },
    created: function(){
    },
    mounted: async function(){
        ...
        try{
            var input = {
                url: base_url + '/dropbox/client_info',
                method: "GET",
                api_key: this.apikey
            };
            var result = await do_http(input);
